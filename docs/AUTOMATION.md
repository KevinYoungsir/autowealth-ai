...
> **注意**：v0.17.1 的核心 Release Workflow 不读取 PyPI Token，也不上传 PyPI。
...
v0.17.1 默认关闭 PyPI 发布。`release.yml` 不读取 `PYPI_API_TOKEN`，也不执行
...
完整人工发布顺序见 `docs/release-process.md`。v0.17.1 不上传 PyPI。
...
`git push origin v0.17.1` 是显式发布操作，必须另行获得授权。正式 tag 不得删除、
...
```bash
git show v0.17.1
git branch -r --contains v0.17.1
python scripts/verify_release_metadata.py --expected-version 0.17.1 --tag v0.17.1
```
