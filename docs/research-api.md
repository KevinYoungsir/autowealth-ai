...
新 run 的 manifest 可增量包含 `macro_validation_diagnostics`。现有 run detail
通过 `manifest` 原样只读暴露；报告仅在 `macro_review.evidence` 返回压缩版本。
旧 run 缺失字段显示 `absent`，损坏字段显示 `invalid`，两者均不改变 HTTP、
原宏观复核状态、risk severity、warning 或运行状态。没有新增 endpoint，也没有
