...
新运行默认在宏观数据加载后、现有 as-of 与 scoring 前执行 shadow validation，
并在 `run_manifest.json` 可选写入 `macro_validation_diagnostics`。validator 使用展开
副本，不替换或过滤原始宏观 DataFrame；关闭校验或校验自身异常时，warning、
`run_status`、宏观 multiplier、持仓、指标和曲线均保持原行为。异常只生成脱敏、
有界的 `invalid` diagnostics，不阻止其他 artifacts 发布。

该字段不升级 artifact schema，也不增加必需 artifact。旧 run 缺少字段继续兼容。
当前校验仅用于审计，尚未强制阻断无效宏观记录进入现有 scoring，也未实现单位、
频率或正式交易日历验证。完整契约见 `docs/macro-valuation-contract.md`。

Historical Valuation 本阶段只有 schema、availability 和 provider protocol，未接入
本流水线、value factor、真实供应商、cache 或 provider chain。记录入口只接受六位
canonical symbol，provider code 只能在 adapter 内转换；请求必须携带 symbol、窗口和
`as_of_date`，晚于 cutoff 发布的记录不进入 PIT 可用结果。

该契约要求显式 `observation_date` 与 `available_date`，但不能证明供应商日期真实，
也不能自动识别把当前 snapshot 配上伪造历史日期或重复值形成的假历史序列。未来
生产 provider 必须提供来源、版本与 PIT 证据并通过真实历史序列 acceptance tests；
当前阶段不能声称已解决历史估值数据可得性或当前值回填识别。
