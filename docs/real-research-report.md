...
- 新 run 的数据质量证据增量返回 structured warning 状态、schema version、完整
  结构化列表、severity 计数和 scope 计数；旧 run 显示 `absent`，损坏字段显示
  `invalid`，两种情况都继续保留 raw warning。
- Warning severity 不等于 report risk severity，结构化元数据不会改变风险评分、
  风险等级、`partial_success` 或 benchmark `unavailable`。
...
新 run 可在 manifest 中包含 `macro_validation_diagnostics`。确定性报告只把其中
状态、原因、记录计数、有效记录比例、指标数量和 PIT 聚合计数压缩加入
`macro_review.evidence`。它不会改变宏观复核原有 `available` 或
